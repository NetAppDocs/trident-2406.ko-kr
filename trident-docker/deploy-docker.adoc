...
.필요한 것
를 link:prereqs-docker.html["배포를 위한 사전 요구 사항"]참조하십시오. 필수 구성 요소가 충족되었는지 확인한 후 Astra Trident를 배포할 준비가 되었습니다.
...
. 관리 플러그인 시스템을 사용하여 Astra Trident를 시작합니다.  `<version>`사용 중인 플러그인 버전(xxx.xx.x)으로 바꿉니다.
+
[listing]
----
