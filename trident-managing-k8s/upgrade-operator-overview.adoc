...
Astra Trident를 설치하고 업그레이드하는 link:../trident-get-install/kubernetes-deploy.html["Trident 연산자를 사용할 때의 이점"] 중 하나는 기존에 마운트된 볼륨에 영향을 주지 않고 Astra Trident 및 Kubernetes 객체를 자동으로 처리한다는 점입니다. 이러한 방식으로 Astra Trident는 다운타임 없이 업그레이드를 지원하거나 link:https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/["_롤링 업데이트 _"^]를 지원할 수 있습니다. 특히, Trident 오퍼레이터는 다음을 위해 Kubernetes 클러스터와 통신합니다.
...
TIP: Kubernetes 클러스터의 Astra Trident 아키텍처에 대한 자세한 내용은 link:../trident-get-started/intro.html#astra-trident-architecture["Astra Trident 아키텍처"]을 참조하십시오.
