...
NOTE: 네임스페이스 범위 연산자를 사용하여 설치된 Astra Trident(버전 20.07~20.10)에서 업그레이드하려면 설치된 Astra Trident 버전에 대한 업그레이드 지침을 사용하십시오.
...
.단계
. link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["Helm을 사용하여 Astra Trident를 설치했습니다"]한 경우  `helm upgrade trident netapp-trident/trident-operator --version 100.2406.0`를 사용하여 한 단계로 업그레이드할 수 있습니다. Helm 리포지토리를 추가하지 않았거나 업그레이드에 사용할 수 없는 경우:
+
