...
Astra Trident에서 LUKS로 암호화된 볼륨은 에서 권장하는 AES-XTS-plan64 사이퍼 및 모드를 사용합니다link:https://csrc.nist.gov/publications/detail/sp/800-38e/final["NIST"^].
...
LUKS 볼륨을 가져오려면 백엔드에서 로(`true` 설정해야 `luksEncryption` 합니다.  `luksEncryption`(`false`다음 예에 표시된 것처럼 볼륨이 LUKS (`true`규정을 준수하는지 여부에 대해서는 Astra Trident에 지시합니다.
